...
## 状态面板
![dashboard](https://cloud-1251017115.cos.ap-guangzhou.myqcloud.com/CPMS/maintenance/%E5%BE%AE%E4%BF%A1%E6%88%AA%E5%9B%BE_20200514160657.png)
...
此外还有一个`next-trigger-alert-in-minute`参数，用于控制下次告警时间。也就是说如果触发告警之后，若告警一直未恢复，则隔多久进行下一次告警。如果不配置，默认1天之后会再次告警（若未恢复）。

`望风塔`是一个非常好用的告警平台，sync2es告警使用了第三方`望风塔`提供支持，使用之前需要到[望风塔](http://www.wangfengta.com)进行简单的配置。
1. 登陆[望风塔](http://www.wangfengta.com)
...
详情：空闲时间：${idleTime}分钟，数据库：${dbName}，表名称：${tables}，index名称：${indexName}
